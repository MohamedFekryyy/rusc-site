#!/bin/sh
# First setup of the codes service on Fly (README.md). Run from this folder,
...
has_app rusc-codes || fly apps create rusc-codes --org "$ORG"

# Its own database role, which can only read Cal's booking tables.
if ! has_secret rusc-codes DATABASE_URL; then
	umask 077
...
SQL
	...
	printf 'DATABASE_URL=postgresql://rusc_codes:%s@rusc-cal-db.internal:5432/cal\n' "$password" |
		fly secrets import --stage -a rusc-codes
	unset password
fi
